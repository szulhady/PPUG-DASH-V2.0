...
router.get("/", (req, res, next) => {
 ...
 const val=req.query.val;
  ...
  var q = `SELECT MIN(${val}) as min,MAX(${val}) as max, AVG(${val}) as avg,timestamp, HOUR(timestamp) AS hour, DATE_FORMAT(timestamp, "%d/%c/%y") AS date FROM ppug_data WHERE PID="${station}" && DATE_FORMAT(timestamp, "%e/%c/%Y")="${today}" GROUP BY date;`;
